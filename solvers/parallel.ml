...
let pmap ?(processes = 4) ?(bsize = 0) f input output =
  ...
  let end_idx start_idx =
    min (Array.length output - 1) (start_idx + bsize - 1)
  in
  let next_idx, total_computed = (ref 0, ref 0) and in_streams = ref [] in
  Gc.compact () ;
  while !total_computed < Array.length output do
    (* Spawn processes *)
    while
      !next_idx < Array.length output && List.length !in_streams < processes
    do
    ...
    let recvs =
      Unix.select
        ~read:(List.map !in_streams ~f:fst)
    ...
    in_streams :=
      List.filter
        ~f:(fun (stream, _) -> not (List.mem ~equal:( = ) recvs.read stream))
        !in_streams
  done ;
  output
...
let parallel_map ~nc l ~f =
  ...
  let output_array =
    pmap
      ~processes:(min (Array.length input_array) nc)
      ~bsize:1
      (fun x -> Some (f x))
      (Array.get input_array) output_array
  in
  Out_channel.flush stdout ;
...
let parallel_work ~nc ?(chunk = 0) ~final actions =
  ...
  then (
    ...
    while !finished_actions < number_of_actions do
      (* Spawn processes *)
      while
        List.length !remaining_actions > 0 && List.length !in_streams < nc
      do
        let rd, wt = Unix.pipe () in
        match Unix.fork () with
        | `In_the_child ->

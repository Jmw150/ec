...
let string_of_categorized_grammar g =
  ...
  | v0 :: vs, p0 :: ps ->
      ...
      let s =
        "When starting a new program, the production probabilities are:\n"
        ^ "logVariable = "
        ^ Float.to_string (variable_value v0)
        ^ "\n"
        ^ (p0
          |> List.map ~f:(fun (f, t, l) ->
                 Printf.sprintf "%f\t%s\t%s" (variable_value l)
                   (string_of_type t) (string_of_fragment f)
             )
          |> join ~separator:"\n"
          )
        ^ (List.zip_exn vs ps
          |> List.mapi ~f:(fun j (v, p) ->
                 "\nWhen starting an argument to fragment "
                 ^ string_of_fragment (List.nth_exn inventory j)
                 ^ ", the production probabilities are:\n"
                 ^ "logVariable = "
                 ^ Float.to_string (variable_value v)
                 ^ "\n"
                 ^ (p
                   |> List.map ~f:(fun (f, t, l) ->
                          Printf.sprintf "%f\t%s\t%s" (variable_value l)
...
let categorized_of_fragment_grammar (f : fragment_grammar) =
  ...
  {
    logVariables =
      1 -- number_of_productions |> List.map ~f:(fun _ -> random_variable ());
    productionProbabilities =
      1 -- number_of_productions
      |> List.map ~f:(fun _ ->
             f.fragments
             |> List.map ~f:(fun (f, t, _) -> (f, t, random_variable ()))
         );
  }
...
let likelihood_under_sfg (g : sfg) (request : tp) (expression : program) :
  ...
  (* Any chain of applications could be broken up at any point by a
     ...
     Example: (+ 1 2) -> [(+,[1,2]), (+1,[2])] *)
  let rec possible_application_parses (p : program) :
      (program * program list) list =
    match p with
    | Apply (f, x) ->
        [ (p, []) ]
        @ (possible_application_parses f
          |> List.map ~f:(fun (fp, xp) -> (fp, xp @ [ x ]))
          )
    | _ -> [ (p, []) ]
  in

  let unifying_fragments
      (production : int)
      (environment : tp list)
      (request : tp)
      context : (int * fragment * tp * tContext * variable) list =
    ...
    let candidates =
      fragments
      @ List.mapi ~f:(fun j t -> (FIndex j, t, logVariable)) environment
    ...
    in
    let z =
      List.map ~f:(fun (_, _, _, _, ll) -> ll) candidates |> log_soft_max
    in
  ...
  let rec likelihood
      ...
      (request : tp)
      (p : program)
      (production : int) : tContext * variable =
    ...
    | _ ->
        (* not a function so must be an application *)
        (* fragments we might match with based on their type *)
        let candidates =
          unifying_fragments production environment request context
        in
        ...
        possible_application_parses p
        |> List.map ~f:(fun (f, arguments) ->
               List.map candidates
                 ~f:(fun (candidate_index, candidate, unified_type, context, ll)
                    ->
                   try
                     ...
                     let context, fragment_request =
                       pad_type_with_arguments context (List.length arguments)
                         request
                     in
                     let context =
                       unify context fragment_request fragment_type
                     in
                     let fragment_type, context =
                       chaseType context fragment_type
                     in

                     let argument_types, _ =
                       arguments_and_return_of_type fragment_type
                     in
                     if not (List.length argument_types = List.length arguments)
                     then (
                       Printf.printf "request: %s\n" (string_of_type request) ;
                       Printf.printf "program: %s\n" (string_of_program p) ;
                       Printf.printf "F = %s, xs = %s\n" (string_of_program f)
                         (arguments
                         |> List.map ~f:string_of_program
                         |> join ~separator:" ;; "
                         ) ;
                     ...
                     let arguments =
                       List.map holes ~f:(fun (_, h) -> h)
                       @ List.map (FreeMap.to_alist bindings)
                           ~f:(fun (_, (_, binding)) -> binding
                         )
                       @ arguments
                     in
                     let argument_types =
                       List.map holes ~f:(fun (ht, _) -> ht)
                       @ List.map (FreeMap.to_alist bindings)
                           ~f:(fun (_, (binding, _)) -> binding
                         )
                       @ argument_types
                     in
                       ...
                       List.fold_right (List.zip_exn arguments argument_types)
                         ...
                         ~f:(fun (argument, argument_type) (ll, context) ->
                           let context, argument_likelihood =
                             likelihood context environment argument_type
                               argument candidate_index
                           in
                           (ll +& argument_likelihood, context)
                       )
                     in
                     (Some context, application_likelihood)
                   with
                   | FragmentFail -> (None, ~$Float.neg_infinity)
                   | UnificationFailure -> assert false
               )
           )
        |> List.concat
        |> (* Accumulate the probabilities from each parse. All of the contexts should be equivalent. *)
        List.fold_right ~init:(context, ~$Float.neg_infinity)
          ~f:(fun (mayBeContext, ll) (oldContext, acc) ->
            match mayBeContext with
            | None ->
                assert (is_invalid (ll.data |> get_some)) ;
                (oldContext, acc)
            | Some c when is_valid (ll.data |> get_some) ->
                (c, log_soft_max [ acc; ll ])
...
let estimate_categorized_fragment_grammar
    (fg : fragment_grammar)
    (frontiers : frontier list) =
  let frontiers =
    frontiers |> List.filter ~f:(fun f -> List.length f.programs > 0)
  in
  ...
  let joint =
    ...
    |> List.map ~f:(fun f ->
           f.programs
           |> List.map ~f:(fun (p, _) -> likelihood_under_sfg g f.request p)
           |> log_soft_max
       )
    |> fold1 ( +& )
  in
  let parameters =
    g.logVariables
    @ (g.productionProbabilities
      |> List.map ~f:(List.map ~f:(fun (_, _, q) -> q))
      |> List.concat
      )
  in

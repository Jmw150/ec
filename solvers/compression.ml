...
let inside_outside ~pseudoCounts g (frontiers : frontier list) =
  ...
  let update g =
    let weighted_summaries =
      ...
      |> List.map ~f:(fun ss ->
             let log_weights =
               ss |> List.map ~f:(fun (l, s) -> l +. summary_likelihood g s)
             in
             let z = lse_list log_weights in
             List.map2_exn log_weights ss ~f:(fun lw (_, s) ->
                 (exp (lw -. z), s)
             )
         )
      |> List.concat
    ...
    in
    let actual p =
      match Hashtbl.find s.use_frequency p with
      | None -> 0.
      | Some f -> f
    in

    {
      g with
      logVariable =
        log (actual (Index 0) +. pseudoCounts)
        -. log (possible (Index 0) +. pseudoCounts);
      library =
        g.library
        |> List.map ~f:(fun (p, t, _, u) ->
               let l =
                 log (actual p +. pseudoCounts)
                 -. log (possible p +. pseudoCounts)
               in
               (p, t, l, u)
           );
    }
  ...
  ( g,
    ...
    |> List.map ~f:(fun ss ->
           ss
           |> List.map ~f:(fun (l, s) -> l +. summary_likelihood g s)
           |> lse_list
       )
    |> fold1 ( +. )
  )
...
let grammar_induction_score ~aic ~structurePenalty ~pseudoCounts frontiers g =
  ...
  ( g,
    ...
    -. structurePenalty
       *. (g.library
          |> List.map ~f:(fun (p, _, _, _) -> production_size p)
          |> sum
          |> Float.of_int
          )
  )
...
let eta_long request e =
  ...
  let rec visit request environment e =
    ...
    | Abstraction b when is_arrow request ->
        Abstraction
          (visit (right_of_arrow request)
             (left_of_arrow request :: environment)
             b
          )
...
let normalize_invention i =
  ...
  let abstracted =
    List.fold_right mapping ~init:renamed ~f:(fun _ e -> Abstraction e)
  in
  ...
  let applied_invention =
    List.fold_left ~init:closed
      (List.range ~start:`exclusive ~stop:`inclusive ~stride:(-1)
         (List.length mapping) 0
      )
      ~f:(fun e i ->
        Apply (e, Index (List.Assoc.find_exn ~equal:( = ) mapping i))
      )
  in
  ...
  fun request e ->
    ...
    | UnificationFailure ->
        ...
        then (
          ...
          Printf.eprintf
            "Going to proceed as if the rewrite had failed - but look into \
             this because it could be a bug.\n" ;
          flush_everything ()
        ) ;
        let normal_original = e |> beta_normal_form ~reduceInventions:true in
        let normal_rewritten =
          e |> visit |> beta_normal_form ~reduceInventions:true
        in
...
let compression_worker connection ~inline ~arity ~bs ~topK g frontiers =
  ...
  let frontier_indices : int list list =
    time_it ~verbose:!verbose_compression "(worker) calculated version spaces"
      (fun () ->
        !frontiers
        |> List.map ~f:(fun f ->
               f.programs
               |> List.map ~f:(fun (p, _) ->
                      incorporate v p |> n_step_inversion v ~inline ~n:arity
                  )
           )
    )
  in
  ...
  let candidates : int list list =
    time_it ~verbose:!verbose_compression "(worker) proposed candidates"
      (fun () ->
        let reachable : int list list =
          frontier_indices |> List.map ~f:(reachable_versions v)
        in
        let inhabitants : int list list =
          reachable
          |> List.map ~f:(fun indices ->
                 List.concat_map
                   ~f:(snd % minimum_cost_inhabitants cost_table)
                   indices
                 |> List.dedup_and_sort ~compare:( - )
  ...
  let candidate_scores : float list =
    time_it ~verbose:!verbose_compression "(worker) beamed version spaces"
      (fun () -> beam_costs' ~ct:cost_table ~bs candidates frontier_indices
    )
  in

  send candidate_scores ;
  (* I hope that this leads to garbage collection *)
  let candidate_scores = () and cost_table = () in
  Gc.compact () ;

  let rewrite_frontiers invention_source =
    time_it ~verbose:!verbose_compression "(worker) rewrote frontiers"
      (fun () ->
        time_it ~verbose:!verbose_compression "(worker) gc during rewrite"
          Gc.compact ;
          ...
          List.map !frontiers ~f:(fun frontier ->
              let programs' =
                List.map frontier.programs ~f:(fun (originalProgram, ll) ->
                    let index =
                      incorporate v originalProgram
                      |> n_step_inversion ~inline v ~n:arity
                    in
                    let program =
                      minimal_inhabitant ~intersectionTable new_cost_table
                        ~given:(Some i) index
                      |> get_some
                    in
                    let program' =
                      try rewriter frontier.request program with
                      | EtaExpandFailure -> originalProgram
                    in
                    (program', ll)
                )
              in
  ...
  let batched_rewrite inventions =
    time_it ~verbose:!verbose_compression "(worker) batched rewrote frontiers"
      (fun () ->
        Gc.compact () ;
        let invention_indices : int list =
          inventions |> List.map ~f:(incorporate v)
        in
        let frontier_indices : int list list =
          !frontiers
          |> List.map ~f:(fun f ->
        ...
        let refactored =
          batched_refactor ~ct:(empty_cost_table v) invention_indices
            frontier_indices
        in
        Gc.compact () ;
        List.map2_exn refactored inventions ~f:(fun new_programs invention ->
            let rewriter = rewrite_with_invention invention in
            List.map2_exn new_programs !frontiers
              ~f:(fun new_programs frontier ->
                let programs' =
                  List.map2_exn new_programs frontier.programs
                    ~f:(fun program (originalProgram, ll) ->
                      ...
                      then (
                        ...
                        Printf.eprintf
                          "This has never occurred before! Definitely send \
                           this to Kevin, if this occurs it is a terrifying \
                           bug.\n" ;
                        assert false
                      ) ;
                      let program' =
  ...
  let final_rewrite invention =
    ...
    time_it ~verbose:!verbose_compression "(worker) did final inversion"
      (fun () ->
        !frontiers
        |> List.iter ~f:(fun f ->
               f.programs
               |> List.iter ~f:(fun (p, _) ->
                      Hashtbl.set frontier_inversions ~key:(incorporate v p)
                        ~data:
                          (n_step_inversion ~inline v ~n:arity (incorporate v p))
                  )
           )
    ) ;
    clear_dynamic_programming_tables v ;
    ...
    let new_cost_table = empty_cheap_cost_table v in
    time_it ~verbose:!verbose_compression "(worker) did final refactor"
      (fun () ->
        List.map !frontiers ~f:(fun frontier ->
            let programs' =
              List.map frontier.programs ~f:(fun (originalProgram, ll) ->
                  ...
                  in
                  let program =
                    minimal_inhabitant new_cost_table ~given:(Some i) index
                    |> get_some
                  in
                  let program' =
                    try
                      rewrite_with_invention invention frontier.request program
                    with
                    | EtaExpandFailure -> originalProgram
                  in
                  (program', ll)
...
let compression_step_master
  ...
  let fork_worker frontiers =
    ...
    let address =
      Printf.sprintf "ipc:///tmp/compression_ipc_%s_%d" timestamp p
    in
    sockets := !sockets @ [ address ] ;

    match Unix.fork () with
    | `In_the_child ->
        compression_worker address ~arity ~bs ~topK ~inline g frontiers
    | _ -> ()
  in
  ...
  let candidates : program list list =
    ...
    |> List.map ~f:(fun s ->
           ...
           let candidate_table =
             { (new_version_table ()) with i2s = candidate_table }
           in
           candidates
           |> List.map
                ~f:(List.map ~f:(singleton_head % extract candidate_table))
       )
    |> List.concat
  in
  let candidates : program list =
    occurs_multiple_times (List.concat candidates)
  in
  ...
  | _ ->
      ...
      let score frontiers candidate =
        let new_grammar =
          uniform_grammar (normalize_invention candidate :: grammar_primitives g)
        in
        let g', s =
        ...
        then (
          ...
          Printf.eprintf "Invention %s : %s\n\tContinuous score %f\n"
            ...
            s ;
          frontiers
          |> List.iter ~f:(fun f -> Printf.eprintf "%s\n" (string_of_frontier f)) ;
          Printf.eprintf "\n" ;
          flush_everything ()
        ) ;
        (g', s)
      ...
      let (g', best_score), best_candidate =
        time_it "Scored candidates" (fun () ->
            List.map2_exn candidates new_frontiers
              ~f:(fun candidate frontiers ->
...
let compression_step
  ...
  let frontier_indices : int list list =
    time_it "calculated version spaces" (fun () ->
        ...
        |> List.map ~f:(fun f ->
               f.programs
               |> List.map ~f:(fun (p, _) ->
                      incorporate v p |> n_step_inversion ~inline v ~n:arity
                  )
           )
    )
  in
  ...
  let candidates =
    ...
    |> List.filter ~f:(fun candidate ->
           ...
           try
             ignore (normalize_invention candidate) ;
  ...
  | _ ->
      ...
      let try_invention_and_rewrite_frontiers (i : int) =
        ...
        with
        | UnificationFailure | DuplicatePrimitive ->
...
let export_compression_checkpoint
    ~nc
    ~structurePenalty
    ~aic
    ~topK
    ...
    |> List.filter ~f:(fun p ->
           not
             (List.mem ~equal:program_equal
                (old_grammar |> grammar_primitives)
                p
             )
       )
    |> singleton_head
  ...
  let illustrate_new_primitive new_grammar primitive frontiers =
    let illustrations =
      ...
      |> List.filter_map ~f:(fun frontier ->
             let best_program =
               (restrict ~topK:1 new_grammar frontier).programs
               |> List.hd_exn
               |> fst
             in
...
let () =
  ...
  (verbose_compression :=
     try j |> member "verbose" |> to_bool with
     | _ -> false
  ) ;

  (factored_substitution :=
  ...
  (collect_data :=
     try j |> member "collect_data" |> to_bool with
     | _ -> false
  ) ;
  if !collect_data then verbose_compression := true ;

  let inline =
    try j |> member "inline" |> to_bool with
    | _ -> true
  in

  let nc =
    try j |> member "CPUs" |> to_int with
    | _ -> 1
  in

  let iterations =
    try j |> member "iterations" |> to_int with
    | _ -> 1000
  in
  ...
  let frontiers =
    j |> member "frontiers" |> to_list |> List.map ~f:deserialize_frontier
  in

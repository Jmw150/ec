...
module CachingTable = struct
  ...
  let set m k v =
    ...
    | None ->
        ...
        let entry = { node_key = k; next = None; previous = m.newest_key } in
        ( match m.newest_key with
  ...
  let check_consistency m =
    let rec forward e =
      ...
      | None -> (
          match m.newest_key with
          | None -> assert false
          | Some this -> assert (this.node_key == e.node_key)
        )
    in
  ...
  let test () =
    ...
    let step () =
      ...
      | Some v' ->
          assert (v = v') ;
          assert (v = Hashtbl.find_exn ground_truth k) ;
          check_consistency m
    in

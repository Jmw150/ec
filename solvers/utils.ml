...
let is_invalid (x : float) =
  x <> x || x = Float.infinity || x = Float.neg_infinity
...
let set_equal c x y =
  let x = List.sort ~compare:c x and y = List.sort ~compare:c y in
...
let merge_a_list ls ~f:c =
  ...
  List.iter ls (fun l ->
      List.iter l (fun (tag, value) ->
          ...
          with
          | Not_found -> ignore (Hashtbl.add merged tag value)
      )
  ) ;
...
let number_of_cores = ref 1

(* number of CPUs *)
let counted_CPUs = ref false

(* have we counted the number of CPUs? *)
...
let cpu_count () =
  try
    ...
    | "Win32" ->
        int_of_string
          (safe_get_some "CPU_count" @@ Sys.getenv "NUMBER_OF_PROCESSORS")
    | _ -> (
        let i = Unix.open_process_in "getconf _NPROCESSORS_ONLN" in
        let close () = ignore (Unix.close_process_in i) in
        try
          Scanf.bscanf (Scanf.Scanning.from_channel i) "%d" (fun n ->
              close () ;
              n
          )
        with
...
let string_proper_prefix p s =
  let rec loop n = n >= String.length p || (p.[n] = s.[n] && loop (n + 1)) in
...
let rec remove_index i l =
  ...
  | 0, x :: xs -> (x, xs)
  | i, x :: xs ->
      let j, ys = remove_index (i - 1) xs in
      (j, x :: ys)
...
let normal s m =
  ...
  let n = sqrt (-2.0 *. log u) *. Float.cos (2.0 *. pi *. v) in
  (s *. n) +. m
...
let random_choice l = Random.int (List.length l) |> List.nth_exn l
...
(* resizable arrays *)
type 'a ra = {
  mutable ra_occupancy : int;
  mutable ra_contents : 'a option Array.t;
}

let empty_resizable () =
  { ra_occupancy = 0; ra_contents = Array.create ~len:10 None }

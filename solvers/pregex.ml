...
and hash_constant = function
  | String cs ->
      List.fold_right ~init:17
        ~f:(fun c a -> Hashtbl.hash (Hashtbl.hash c, a))
        cs
  | c -> Hashtbl.hash c
...
let rec match_regex random (state : match_state) r return =
  ...
  | Constant (String s) ->
      if List.take state.match_target (List.length s) = s then
        return
          {
            state with
            match_target = List.drop state.match_target (List.length s);
          }
  | Constant k -> (
  ...
  | Alt (x, y) ->
      let state' =
        { state with match_likelihood = state.match_likelihood -. log 2. }
      in
...
let match_regex regex s =
  ...
  match_regex
    ...
    regex
    (fun final -> if final.match_target = [] then final_state := Some final) ;
...
let f_kleene a partial =
  ...
  | (Some r, remainder), score ->
      ((Some (Concat (r, Kleene a)), remainder), score +. log 0.5)
...
let f_plus a partial =
  ...
  | (Some r, remainder), score ->
      ((Some (Concat (r, Kleene a)), remainder), score)
...
and consume cont =
  ...
  | None, _ :: _ ->
      [] (* no regular expression and things to consume - not a match *)
  ...
  | Some (Alt (a, b)), str ->
      List.map (consume (Some a, str) @ consume (Some b, str)) ~f:f_alt
...
ignore (primitive "r_dot" (tregex @> tregex) (fun k -> Concat (Constant Dot, k)))
;;
ignore (primitive "r_d" (tregex @> tregex) (fun k -> Concat (Constant D, k))) ;;
ignore (primitive "r_s" (tregex @> tregex) (fun k -> Concat (Constant S, k))) ;;
ignore (primitive "r_w" (tregex @> tregex) (fun k -> Concat (Constant W, k))) ;;
ignore (primitive "r_l" (tregex @> tregex) (fun k -> Concat (Constant L, k))) ;;
ignore (primitive "r_u" (tregex @> tregex) (fun k -> Concat (Constant U, k))) ;;
...
let rec substitute_constant_regex constant p =
  ...
  | Apply (f, x) ->
      Apply
        ( substitute_constant_regex constant f,
...
|> List.iter ~f:(fun i ->
       ...
       in
       let the_primitive =
         primitive name (tregex @> tregex) (fun k ->
             Concat (Constant (String [ i ]), k)
         )
       in
...
let regex_of_program expression : pregex =
  run_lazy_analyzed_with_arguments
    (analyze_lazy_evaluation expression)
    [ empty_regex ]
;;
...
register_special_task "regex"
  (fun extra ?(timeout = 0.001) name task_type examples ->
    assert (task_type = tregex @> tregex) ;
    examples |> List.iter ~f:(fun (xs, _) -> assert (List.length xs = 0)) ;

    let observations : char list list =
      examples |> List.map ~f:(fun (_, y) -> y |> magical)
    in

...
let string_of_frontier f =
  join ~separator:"\n"
    ...
    @ (f.programs
      |> List.map ~f:(fun (p, l) ->
             Printf.sprintf "%f\t%s" l (string_of_program p)
         )
      )
    )
        ...
        ( "programs",
          `List
            ...
            |> List.map ~f:(fun (p, l) ->
                   `Assoc
                     [
                       ("program", `String (string_of_program p));
...
let string_of_state { skeleton; context; path; cost } =
  ...
  in
  Printf.sprintf "{cost=%f; skeleton=%s; path=%s; env=[%s];}" cost
    (string_of_program skeleton)
  ...
  | _ ->
      ...
      |> List.map ~f:(fun (candidate, argument_types, context, ll) ->
             ...
             let argument_requests =
               ...
               | Index _ ->
                   argument_types
                   |> List.map ~f:(fun at -> (at, cg.variable_context))
               | _ ->
                   List.Assoc.find_exn cg.contextual_library candidate
                     ~equal:program_equal
             ...
             | first_argument :: later_arguments ->
                 ...
                 {
                   skeleton =
                     modify_skeleton state.skeleton application_template
                       state.path;
                   cost = state.cost -. ll;
                   free_parameters = state.free_parameters + new_free_parameters;
                   context;
                   path =
                     state.path
                     @ List.map ~f:(fun _ -> L) later_arguments
                     @ [ R ];
                 }
         )
...
let rec enumerate_programs'
  ...
  then ()
    ...
    | TCon ("->", [ argument_type; return_type ], _) ->
        ...
        enumerate_programs' ~maximumDepth ~parent:None cg g context return_type
          newEnvironment lower_bound upper_bound ~maxFreeParameters
          (fun body newContext ll fp ->
               ...
               then ()
                 ...
                 then ()
                   ...
                   enumerate_contextual_applications
                     ...
                     ~maxFreeParameters:(maxFreeParameters - fp) cg context
                     environment argument_requests candidate (lower_bound +. ll)
                     (upper_bound +. ll) (fun p k al fp' ->
                       callBack p k (ll +. al) (fp + fp')
                   )
           )

and enumerate_contextual_applications
    ?(maximumDepth = 9999)
    (cg : contextual_grammar)
    (context : tContext)
    (environment : tp list)
    (argument_requests : (tp * grammar) list)
    (f : program)
    ?(originalFunction = f)
    ?(argumentIndex = 0)
    ?(maxFreeParameters = 99)
  ...
  then ()
    ...
    | (first_argument, first_grammar) :: later_arguments ->
        ...
        enumerate_programs' ~maximumDepth ~maxFreeParameters
          ...
          (fun a k ll fp ->
            let a = Apply (f, a) in
            enumerate_contextual_applications ~originalFunction
              ~argumentIndex:(argumentIndex + 1) ~maximumDepth
              ~maxFreeParameters:(maxFreeParameters - fp) cg k environment
              later_arguments a (lower_bound +. ll) (upper_bound +. ll)
...
let dfs_around_skeleton cg ~maxFreeParameters ~lower_bound ~upper_bound state k
  ...
  let rec around e abstraction_depth ?(parent = None) =
    ...
    | Apply (f, x) when free x && not (free f) ->
        let around_argument =
          around ~parent:(Some (parent_index e)) x abstraction_depth
        in
        fun context l u mfp k ->
    ...
    | Primitive (t, "??", g) ->
        let g = !(g |> magical) in
        let environment =
          List.drop environment (List.length environment - abstraction_depth)
        in
        fun context l u mfp k ->
          let context, t = applyContext context t in
          (* Printf.printf "Enumerating around type %s mfp = %d\n"
           *   (string_of_type t) (mfp); *)
          enumerate_programs' ~parent cg g context t environment l u
            ~maxFreeParameters:mfp k
    | _ -> assert false
  in

  around ~parent:None state.skeleton 0 state.context (lower_bound -. state.cost)
    (upper_bound -. state.cost) (maxFreeParameters - state.free_parameters)
...
let multicore_enumeration
  ...
  let finished, fringe =
    best_first_enumeration ~lower_bound:(Some lb) ~upper_bound:(Some ub)
  ...
  let continuation s =
    dfs_around_skeleton cg ~lower_bound:lb ~upper_bound:ub ~maxFreeParameters s
      strip_context
  in
...
let enumerate_programs
  ...
  let definitely_recursive =
    grammar_has_recursion number_of_arguments cg.no_context
  in
...
let test_best_enumeration () =
  ...
  enumerate_programs ~maxFreeParameters:mfp
    ...
    ~nc:(Sys.argv.(1) |> Int.of_string)
    (g |> make_dummy_contextual)
    request 0.
    (Sys.argv.(2) |> Float.of_string)
    k

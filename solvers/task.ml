              ...
              try
                match
                  run_for_interval timeout (fun () ->
                      run_lazy_analyzed_with_arguments p xs = y
                  )
                with
                | Some true -> loop e
                | _ -> false
      ...
      let bx =
        run_for_interval timeout (fun () ->
            ...
            then None
              ...
              | None ->
                  let bx =
                    LogoLib.LogoInterpreter.turtle_to_array_and_cost x 28
                  in
...
register_special_task "LOGO" (fun extras ?(timeout = 0.001) name ty examples ->
    ...
    let cost_matters =
      try extras |> member "costMatters" |> to_bool with
      | _ -> assert false
    in
...
register_special_task "differentiable"
  (fun extras ?(timeout = 0.001) name ty examples ->
    let open Yojson.Basic.Util in
    let maybe_float name default =
      try extras |> member name |> to_number with
      | _ -> default
    in
    let maybe_int name default =
      try extras |> member name |> to_int with
      | _ -> default
    in
    ...
    let lossThreshold =
      try Some (extras |> member "lossThreshold" |> to_number) with
      | _ -> None
    in
    let clipOutput =
      try Some (extras |> member "clipOutput" |> to_number) with
      | _ -> None
    in
    let clipLoss =
      try Some (extras |> member "clipLoss" |> to_number) with
      | _ -> None
    in
    let proportional =
      try extras |> member "proportional" |> to_bool with
      | _ -> false
    in
...
let enumerate_for_tasks
    (g : contextual_grammar)
    ?(verbose = true)
    ~maxFreeParameters
    ?(budgetIncrement = 1.)
    ?(lowerBound = 0.)
  ...
  let hits =
    Array.init nt ~f:(fun _ ->
        Heap.create
          ~cmp:(fun h1 h2 ->
            Float.compare
              (h1.hit_likelihood +. h1.hit_prior)
              (h2.hit_likelihood +. h2.hit_prior)
          )
          ()
    )
  in
  ...
  while
    ...
    && List.exists (range nt) ~f:(fun j ->
           Heap.length hits.(j) < maximumFrontier.(j)
       )

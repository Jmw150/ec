...
primitive "get-position" (tobject @> tvector) (fun o -> o.position) ;;
primitive "get-velocity" (tobject @> tvector) (fun o -> o.velocity) ;;
primitive "mass" (tobject @> treal) (fun o -> o.mass) ;;

primitive "*v"
  (treal @> tvector @> tvector)
  (fun r v -> v |> List.map ~f:(fun v' -> v' *& r))
;;

primitive "/v"
  (tvector @> treal @> tvector)
  (fun v r -> v |> List.map ~f:(fun v' -> v' /& r))
;;

primitive "+v"
  (tvector @> tvector @> tvector)
  (fun a b -> List.map2_exn a b ~f:( +& ))
;;
...
primitive "normalize" (tvector @> tvector) (fun v ->
    let l =
      v |> List.map ~f:square |> List.reduce_exn ~f:( +& ) |> square_root
    in
...
primitive "dp"
  (tvector @> tvector @> treal)
  (fun a b -> List.map2_exn a b ~f:( *& ) |> List.reduce_exn ~f:( +& ))
;;

primitive "vector-length" (tvector @> treal) (fun v ->
    v
...
register_special_task "physics" (fun extra ?(timeout = 0.01) name request _ ->
    let open Yojson.Basic.Util in
    let open Timeout in
    let maybe_float name default =
      try extra |> member name |> to_number with
      | _ -> default
    in
    let maybe_int name default =
      try extra |> member name |> to_int with
      | _ -> default
    in
    ...
    let lossThreshold =
      try Some (extra |> member "lossThreshold" |> to_number) with
      | _ -> None
    in
    let clipOutput =
      try Some (extra |> member "clipOutput" |> to_number) with
      | _ -> None
    in
    let clipLoss =
      try Some (extra |> member "clipLoss" |> to_number) with
      | _ -> None
    in
    ...
    let unpack_object j =
      magical
        {
          position = j |> member "position" |> unpack_vector;
    ...
    let loss = polymorphic_sse ~clipOutput ~clipLoss return in
    {
      ...
      log_likelihood =
        (fun expression ->
          ...
          (* Returns loss list *)
          let rec loop : 'a list -> Differentiation.variable list option =
            function
            | [] -> Some []
            | (xs, y) :: e -> (
                try
                  match
                    run_for_interval timeout (fun () ->
                        run_lazy_analyzed_with_arguments p xs
                    )
                  with
                  | None -> None
                  | Some prediction -> (
                      ...
                      | None -> None
                      | Some later_loss -> (
                          try Some (loss prediction y :: later_loss) with
                          | DifferentiableBadShape -> None
                        )
                    )
                with
                | UnknownPrimitive n ->

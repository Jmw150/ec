...
let rec program_to_combinator = function
  ...
  | Abstraction b when not (variable_is_bound b) ->
      Apply (cK, program_to_combinator b)
  | Abstraction (Abstraction b) ->
      let b =
        program_to_combinator (Abstraction b) |> shift_free_variables (-1)
      in
  ...
  | Abstraction (Apply (f, x)) ->
      ...
      Apply
        ( Apply (cS, program_to_combinator (Abstraction f)),
...
let combinator_to_program p =
  let rec combinator_to_program_ = function
    ...
    | Primitive (_, "S") ->
        Abstraction
          (Abstraction
             (Abstraction
                (Apply (Apply (Index 2, Index 0), Apply (Index 1, Index 0)))
             )
          )
    | Primitive (_, "C") ->
        Abstraction
          (Abstraction (Abstraction (Apply (Apply (Index 2, Index 0), Index 1))))
    | Primitive (_, "B") ->
        Abstraction
          (Abstraction (Abstraction (Apply (Index 2, Apply (Index 1, Index 0)))))
    | Invented (t, p) -> Invented (t, p)
  ...
  let rec reduce p =
    ...
    | _ -> None
  and substitute ?(height = 0) value body =
    match body with
    | Apply (f, x) ->
        Apply (substitute ~height value f, substitute ~height value x)
    | Abstraction b -> Abstraction (substitute ~height:(height + 1) value b)
    | Index j when j = height -> shift_free_variables height value
    | _ -> body
  in
  let rec repeatedly_reduce p =
    match reduce p with
    | None -> p
...
let test_combinator () =
  [ cS; cI; cB; cC; cK ]
  |> List.iter ~f:(fun (Primitive (t, n)) ->
         Printf.printf "%s : %s\n" n (string_of_type t)
     ) ;
  List.iter
    [
      Abstraction (Abstraction (Index 1));
      Abstraction (Abstraction (Index 0));
      Abstraction (Apply (Apply (primitive_multiplication, Index 0), Index 0));
      Abstraction
        (Abstraction (Apply (Apply (primitive_multiplication, Index 0), Index 1))
        );
      Abstraction
        (Abstraction
           (Abstraction
              (Apply (Apply (primitive_multiplication, Index 0), Index 2))
           )
        );
      Abstraction
        (Abstraction
           (Abstraction
              (Apply (Apply (primitive_multiplication, Index 0), Index 1))
           )
        );
    ]
    ~f:(fun p ->

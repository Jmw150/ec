...
let load_problems channel =
  ...
  let timeout =
    try j |> member "programTimeout" |> to_number with
    | _ ->
  ...
  let maxParameters =
    try j |> member "maxParameters" |> to_int with
    | _ -> 99
  in
    ...
    |> List.map ~f:(fun j ->
           ...
           let task =
             ( try
                 ...
                 | None ->
                     Printf.eprintf
                       " (ocaml) FATAL: Could not find handler for %s\n" special ;
                     exit 1
               with
             | _ -> supervised_task
             )
  ...
  let _ =
    try shatter_factor := j |> member "shatter" |> to_int with
    | _ -> ()
  in

  let lowerBound =
    try j |> member "lowerBound" |> to_number with
    | _ -> 0.
  in

  let upperBound =
    try j |> member "upperBound" |> to_number with
    | _ -> 99.
  in

  let budgetIncrement =
    try j |> member "budgetIncrement" |> to_number with
    | _ -> 1.
  in

  let timeout = j |> member "timeout" |> to_number in
  let nc =
    try j |> member "nc" |> to_int with
    | _ -> 1
  in
  ( tf,
    g,
...
let _ =
  let tf, g, lowerBound, upperBound, budgetIncrement, mfp, nc, timeout, verbose
      =
    load_problems Pervasives.stdin
  in
  let solutions, number_enumerated =
    enumerate_for_tasks ~maxFreeParameters:mfp ~lowerBound ~upperBound
      ~budgetIncrement ~verbose ~nc ~timeout g tf
  in

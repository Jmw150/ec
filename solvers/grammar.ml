...
type grammar = {
  logVariable : float;
  library :
    (program * tp * float * (tContext -> tp -> tContext * tp list)) list;
    ...
    library =
      List.map primitives ~f:(fun p ->
          ...
          | Primitive (t, _, _) ->
              ( p,
                t,
                0.0 -. log (float_of_int (List.length primitives)),
                compile_unifier t
              )
...
let strip_grammar { logVariable; continuation_type; library } =
  {
    ...
    continuation_type;
    library =
      library |> List.map ~f:(fun (p, t, l, u) -> (strip_primitives p, t, l, u));
  }
...
let unifying_expressions g environment request context :
...
let show_summary s =
  join ~separator:"\n"
    ...
    @ (Hashtbl.to_alist s.normalizer_frequency
      |> List.map ~f:(fun (n, f) ->
             let n =
               n |> List.map ~f:string_of_program |> join ~separator:","
             in
...
let record_likelihood_event likelihood_summary actual possibles =
  ...
  likelihood_summary.likelihood_constant <-
    likelihood_summary.likelihood_constant +. constant ;
...
let make_likelihood_summary g request expression =
  ...
  let rec summarize (r : tp) (environment : tp list) (p : program) : unit =
    ...
    | _ -> (
        ...
        | f :: xs -> (
            match
              List.find candidates ~f:(fun (candidate, _, _, _) ->
                  program_equal candidate f
              )
            with
            ...
            | Some (_, argument_types, newContext, functionLikelihood) ->
                ...
                record_likelihood_event s f
                  (candidates
                  |> List.map ~f:(fun (candidate, _, _, _) -> candidate)
                  ) ;
  ...
  let ls =
    ...
    @ (cg.contextual_library
      |> List.map ~f:(fun (e, gs) ->
             ...
             |> List.mapi ~f:(fun i g ->
                    [
                      Printf.sprintf "Parent %s, argument index %i:"
                        (string_of_program e) i;
                      string_of_grammar g;
                      "";
                    ]
                )
        ...
        let argument_type = return_of_type argument_type in
        {
          ...
          library =
            ...
            |> List.filter ~f:(fun (_, child_type, _, _) ->
                   ...
                   try
                     let k, child_type =
                       instantiate_type empty_context child_type
                     in
...
let make_dummy_contextual g =
  {
    ...
    contextual_library =
      g.library
      |> List.map ~f:(fun (e, t, _, _) ->
             (e, arguments_of_type t |> List.map ~f:(fun _ -> g))
         );
  }
...
let deserialize_grammar g =
  ...
  let productions =
    ...
    |> List.map ~f:(fun p ->
           ...
           let e =
             parse_program source |> safe_get_some ("Error parsing: " ^ source)
           in
           let t =
             try infer_program_type empty_context [] e |> snd with
             | UnificationFailure -> raise (Failure ("Could not type " ^ source))
           in
  ...
  let continuation_type =
    try Some (g |> member "continuationType" |> deserialize_type) with
    | _ -> None
  in
  ...
  let open Yojson.Basic.Util in
  {
    ...
    contextual_library =
      ...
      |> List.map ~f:(fun production ->
             ...
             let e =
               try e |> parse_program |> get_some with
               | _ ->
             ...
             let children =
               ...
               |> member "arguments"
               |> to_list
               |> List.map ~f:deserialize_grammar
             in
             (e, children)
         );
  }
...
let deserialize_contextual_grammar g =
  try deserialize_grammar g |> make_dummy_contextual with
  | _ -> deserialize_contextual_grammar g

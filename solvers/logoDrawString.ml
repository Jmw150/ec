...
let _ =
  ...
  let pretty =
    try to_bool (member "pretty" j) with
    | _ -> false
  in
  let smooth_pretty =
    try to_bool (member "smoothPretty" j) with
    | _ -> false
  in
  let timeout =
    try to_number (member "timeout" j) with
    | _ -> 0.01
  in
  ...
  let results =
    List.map jobs ~f:(fun j ->
        ...
        let animate =
          try to_bool (member "animate" j) with
          | _ -> false
        in
        ...
        then (
          ...
          | Some export ->
              ...
              List.iteri cs (fun j c ->
                  output_canvas_png ~pretty c size
                    (Printf.sprintf "%s_%09d.png" export j)
              ) ;
              Sys.command
                (Printf.sprintf "convert -delay 1 -loop 0 %s_*.png %s.gif"
                   export export
                ) ;
          ...
          try
            ...
            | Some (c, array, cost) -> (
                ...
                then `String "empty"
                  ...
                  match export with
                  | Some fn ->
                      output_canvas_png ~pretty c size fn ;
                      `String "exported"
                  | None ->
                      `Assoc
                        [
                          ...
                          ( "pixels",
                            `List
                              (List.map
                                 (range (Bigarray.Array1.dim array))
                                 ~f:(fun i -> `Int array.{i})
                              )
                          );
                        ]

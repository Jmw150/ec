...
let primitive_uppercase =
  primitive "uppercase" (tstring @> tstring) String.uppercase

let primitive_lowercase =
  primitive "lowercase" (tstring @> tstring) String.lowercase

let primitive_capitalize =
  primitive "capitalize" (tstring @> tstring) String.capitalize

let primitive_concatenate =
  primitive "concatenate" (tstring @> tstring @> tstring) ( ^ )
...
let primitive_map_string =
  primitive "map-string"
    ((tstring @> tstring) @> tlist tstring @> tlist tstring)
    (fun f l -> List.map ~f l)

let primitive_find_string =
  primitive "find-string"
    (tstring @> tstring @> tint)
    (fun pattern target -> String.index target pattern.[0] |> get_some)
...
let primitive_string_split =
  primitive "split"
    (tstring @> tstring @> tlist tstring)
    (fun d x -> String.split ~on:d.[0] x)

let primitive_string_join =
  primitive "join"
    (tstring @> tlist tstring @> tstring)
...
let _ =
  ...
  (* assert false; *)
  exploration_compression tasks g ~lambda:0.1 ~alpha:4. ~keepTheBest:1 ~arity:2
    10000 5

...
let rec string_of_versions t j =
  ...
  | Void -> "Void"
  | ApplySpace (f, x) ->
      Printf.sprintf "@(%s, %s)" (string_of_versions t f)
        (string_of_versions t x)
...
let rec incorporate t e =
  ...
  | Primitive (_, _, _) | Invented (_, _) ->
      version_terminal t (strip_primitives e)
...
let rec extract t j =
  ...
  | ApplySpace (f, x) ->
      ...
      |> List.concat_map ~f:(fun f' ->
             extract t x |> List.map ~f:(fun x' -> Apply (f', x'))
         )
...
let rec shift_free ?(c = 0) t ~n ~index =
  if n = 0 then
    ...
    | Union indices ->
        union t (indices |> List.map ~f:(fun i -> shift_free ~c t ~n ~index:i))
    | IndexSpace i when i < c -> index (* below cut off - bound variable *)
    | IndexSpace i when i >= n + c ->
        version_index t (i - n) (* free variable *)
    | IndexSpace _ -> t.void
    | ApplySpace (f, x) ->
        version_apply t
          (shift_free ~c t ~n ~index:f)
          (shift_free ~c t ~n ~index:x)
...
let rec shift_versions ?(c = 0) t ~n ~index =
  ...
  if n = 0 then
    ...
    | Union indices ->
        union t
          (indices |> List.map ~f:(fun i -> shift_versions ~c t ~n ~index:i))
    | IndexSpace i when i < c -> index (* below cut off - bound variable *)
    | IndexSpace i when i + n >= 0 ->
        version_index t (i + n) (* free variable *)
    | IndexSpace _ -> t.void
    | ApplySpace (f, x) ->
        version_apply t
          (shift_versions ~c t ~n ~index:f)
          (shift_versions ~c t ~n ~index:x)
    | AbstractSpace b ->
        version_abstract t (shift_versions ~c:(c + 1) t ~n ~index:b)
    | TerminalSpace _ | Universe | Void -> index
...
let rec subtract t a b =
  ...
  | _, Union xs ->
      List.fold_right xs ~init:a ~f:(fun to_remove current ->
          subtract t current to_remove
      )
  ...
  | ApplySpace (f1, x1), ApplySpace (f2, x2) ->
      union t
        [
          version_apply t (subtract t f1 f2) x1;
...
let rec intersection t a b =
  ...
  | Union xs, Union ys ->
      ...
      |> List.concat_map ~f:(fun x ->
             ys |> List.map ~f:(fun y -> intersection t x y)
         )
      |> union t
...
let inline t j =
  ...
  let rec il (arguments : int list) (j : int) : int =
    ...
    | AbstractSpace _ | IndexSpace _ | TerminalSpace (Primitive (_, _, _)) ->
        t.void
    ...
    | TerminalSpace (Invented (_, body)) -> (
        let rec make_substitution used_arguments unused_arguments body =
          ...
          | x :: xs, Abstraction b ->
              make_substitution (x :: used_arguments) xs b
        ...
        let rec apply_substitution ~k arguments expression =
          ...
          | Abstraction b ->
              version_abstract t (apply_substitution ~k:(k + 1) arguments b)
        ...
        | Some (used_arguments, body) ->
            ...
            let remaining_arguments =
              List.drop arguments (List.length used_arguments)
            in
      ...
      | Union u ->
          ...
          |> List.iter ~f:(fun x ->
                 ...
                 |> Hashtbl.iteri ~f:(fun ~key:v ~data:b ->
                        ...
                        | Some stuff ->
                            Hashtbl.set new_mapping ~key:v ~data:(b :: stuff)
          ...
          |> Hashtbl.iteri ~f:(fun ~key:v1 ~data:f ->
                 ...
                 |> Hashtbl.iteri ~f:(fun ~key:v2 ~data:x ->
                        if have_intersection t v1 v2 then
                          Hashtbl.update new_mapping (intersection t v1 v2)
                            ~f:(function
                            | None -> ([ f ], [ x ])
          ...
          |> Hashtbl.iteri ~f:(fun ~key:v1 ~data:f ->
                 ...
                 |> Hashtbl.iteri ~f:(fun ~key:v2 ~data:x ->
                        if have_intersection t v1 v2 then
                          ...
                          match Hashtbl.find new_mapping v with
                          | Some stuff ->
                              Hashtbl.set new_mapping ~key:v ~data:(a :: stuff)
...
let inversion t j =
  ...
  |> List.filter_map ~f:(fun (v, b) ->
         if v = t.universe || index_table t b = IndexSpace 0 then
           None
         else
      ...
      let ri =
        ...
        | _ ->
            ...
            let child_inversions =
              ...
              | AbstractSpace b ->
                  [ version_abstract t (recursive_inversion t b) ]
              | _ -> []
            in
...
let empty_cost_table t =
  {
    function_cost = empty_resizable ();
    argument_cost = empty_resizable ();
    cost_table_parent = t;
  }

let rec minimum_cost_inhabitants ?(given = None) ?(canBeLambda = true) t j :
    float * int list =
  let caching_table =
    if canBeLambda then
      t.argument_cost
      ...
      let c =
        ...
        | _ -> (
            ...
            | Union u ->
                let children =
                  u
                  |> List.map ~f:(minimum_cost_inhabitants ~given ~canBeLambda t)
                in
                let c =
                  children |> List.map ~f:(fun (cost, _) -> cost) |> fold1 min
                in
                if is_invalid c then
                  (c, [])
                else
                  let children =
                    children |> List.filter ~f:(fun (cost, _) -> cost = c)
                  in
                  (c, children |> List.concat_map ~f:(fun (_, p) -> p))
            | AbstractSpace b when canBeLambda ->
                let cost, children =
                  minimum_cost_inhabitants ~given ~canBeLambda:true t b
                in
                ( cost +. epsilon_cost,
            ...
            | ApplySpace (f, x) ->
                let fc, fs =
                  minimum_cost_inhabitants ~given ~canBeLambda:false t f
                in
                let xc, xs =
                  minimum_cost_inhabitants ~given ~canBeLambda:true t x
                in
                if is_invalid fc || is_invalid xc then
                  (Float.infinity, [])
                else
                  ( fc +. xc +. epsilon_cost,
                    fs
                    |> List.map ~f:(fun f' ->
                           xs
                           |> List.map ~f:(fun x' ->
                                  version_apply t.cost_table_parent f' x'
                              )
                       )
                    |> List.concat
                  )
...
let rec minimal_inhabitant_cost
  ...
  | None ->
      let c =
        match given with
        | Some invention
          when have_intersection ~table:intersectionTable t.cost_table_parent
                 invention j ->
            1.
        | _ -> (
            match index_table t.cost_table_parent j with
            ...
            | Union u ->
                ...
                |> List.map
                     ~f:
                       (minimal_inhabitant_cost ~intersectionTable ~given
                          ~canBeLambda t
                       )
                |> fold1 min
            | AbstractSpace b when canBeLambda ->
                epsilon_cost
                +. minimal_inhabitant_cost ~intersectionTable ~given
                     ~canBeLambda:true t b
            | AbstractSpace b -> Float.infinity
            | ApplySpace (f, x) ->
                epsilon_cost
                +. minimal_inhabitant_cost ~intersectionTable ~given
                     ~canBeLambda:false t f
                +. minimal_inhabitant_cost ~intersectionTable ~given
                     ~canBeLambda:true t x
          )
      in
      set_resizable caching_table j (Some c) ;
      c
  ...
  if is_invalid c then
    ...
    let p =
      ...
      | _ -> (
          ...
          | AbstractSpace b ->
              Abstraction
                (minimal_inhabitant ~intersectionTable ~given ~canBeLambda:true
                   t b
                |> get_some
                )
          | ApplySpace (f, x) ->
              Apply
                ( minimal_inhabitant ~intersectionTable ~given
                    ~canBeLambda:false t f
                  |> get_some,
                  minimal_inhabitant ~intersectionTable ~given ~canBeLambda:true
                    t x
...
let relative_function b i =
  match Hashtbl.find b.relative_function i with
  | None -> b.default_function_cost
  | Some c -> c

let relative_argument b i =
  match Hashtbl.find b.relative_argument i with
...
let beam_costs''
    ...
    ~bs
    (candidates : int list)
    (frontier_indices : int list list) : beam option ra =
  ...
  let rec calculate_costs j =
    ...
    | Some bm -> bm
    | None ->
        let default_argument_cost, inhabitants =
          minimum_cost_inhabitants ~canBeLambda:true ct j
        in
        let default_function_cost, _ =
          minimum_cost_inhabitants ~canBeLambda:false ct j
        in
        let bm =
          {
        ...
        | ApplySpace (f, x) ->
            ...
            let domain =
              Hashtbl.keys fb.relative_function
              @ Hashtbl.keys xb.relative_argument
            in
            domain
            |> List.iter ~f:(fun i ->
                   let c =
                     epsilon_cost
                     +. relative_function fb i
                     +. relative_argument xb i
                   in
  ...
  frontier_indices
  |> List.iter ~f:(List.iter ~f:(fun j -> ignore (calculate_costs j))) ;
  caching_table
...
let beam_costs'
    ...
    ~bs
    (candidates : int list)
    (frontier_indices : int list list) : float list =
  let caching_table = beam_costs'' ~ct ~bs candidates frontier_indices in
  let frontier_beams =
    frontier_indices
    |> List.map
         ~f:(List.map ~f:(fun j -> get_resizable caching_table j |> get_some))
  in
  ...
  let score i =
    let corpus_size =
      ...
      |> List.map ~f:(fun bs ->
             bs
             |> List.map ~f:(fun b ->
                    min (relative_argument b i) (relative_function b i)
                )
             |> fold1 min
         )
      |> fold1 ( +. )
...
let beam_costs ~ct ~bs (candidates : int list) (frontier_indices : int list list)
    =
  let scored =
    List.zip_exn (beam_costs' ~ct ~bs candidates frontier_indices) candidates
  in
  scored |> List.sort ~compare:(fun (s1, _) (s2, _) -> Float.compare s1 s2)

let batched_refactor
    ~ct
    (candidates : int list)
    (frontier_indices : int list list) =
  let caching_table =
    beam_costs'' ~ct ~bs:(List.length candidates) candidates frontier_indices
  in
  ...
  let rec refactor ~canBeLambda i j =
    ...
    if List.mem ~equal:( = ) inhabitants i then
      ...
      | ApplySpace (f, x) ->
          Apply (refactor ~canBeLambda:false i f, refactor ~canBeLambda:true i x)
      | Union u ->
          u
          |> minimum_by (fun u' ->
  ...
  |> List.map ~f:(fun i ->
         frontier_indices
         |> List.map ~f:(fun f ->
                f |> List.map ~f:(fun j -> refactor ~canBeLambda:true i j)
            )
     )

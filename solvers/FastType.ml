...
let compile_unifier t =
  ...
  let rec fu f t k : tContext =
    ...
    | _ ->
        raise
          (Failure
             "Fast types do not currently support arity >=3 type constructors"
          )
  in
  ...
  fun context request ->
    ...
    let arguments =
      List.map argument_types ~f:(make_slow next_type_variable')
    in
    let context =
      makeTIDs (!next_type_variable' - next_type_variable) context
    in
...
let test_fast () =
  ...
  library_types
  |> List.iter ~f:(fun library_type ->
         let u = compile_unifier library_type in
         requesting_types
         |> List.iter ~f:(fun request ->
                ...
                Printf.printf "library type: %s\trequesting type: %s\t\n"
                  (string_of_type library_type)
                  (string_of_type request) ;
                Printf.printf "arguments:\t%s\n"
                  (arguments
                  |> List.map ~f:(snd % applyContext k)

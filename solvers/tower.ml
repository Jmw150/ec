...
let empty_tower_state = { hand_position = 0; hand_orientation = 1 }

(* ttower = state -> (state, list of blocks) *)
type tt = tower_state -> tower_state * (int * int * int) list

let tower_sequence (a : tt) (b : tt) : tt =
 fun hand ->
...
ignore
  (primitive "left"
     (tint @> ttower @> ttower)
     (let f : int -> tt -> tt =
       fun (d : int) (k : tt) (hand : tower_state) ->
...
ignore
  (primitive "tower_loopM"
     (tint @> (tint @> ttower @> ttower) @> ttower @> ttower)
     (fun i (f : int -> tt -> tt) (z : tt) : tt ->
       List.fold_right (0 -- (i - 1)) ~f ~init:z
     )
  )
;;

ignore
  (primitive "tower_embed"
     ((ttower @> ttower) @> ttower @> ttower)
     ...
     (fun (d : int) (k : tt) : tt ->
       fun (state : tower_state) ->
        k
          {
            state with
            hand_position = state.hand_position + (state.hand_orientation * d);
          }
     )
  )
;;
...
ignore
  (primitive "reverseHand" (ttower @> ttower) (fun (k : tt) : tt ->
       fun (state : tower_state) ->
...
let show_tower dt =
  ...
  |> List.map ~f:(fun (a, b, c, d) ->
         Printf.sprintf "BLOCK{w=%d, h=%d, x=%d, y=%d}" c d a b
     )
  |> join ~separator:"\n"
        ...
        try
          match
            run_for_interval timeout (fun () ->
                run_lazy_analyzed_with_arguments p
                  [ (fun s -> (s, [])) ]
                  empty_tower_state
                |> snd
            )
...
register_special_task "supervisedTower"

...
type variable = {
  mutable gradient : float option;
  mutable data : float option;
  volatile : bool;
      (* whether this depends on something which is a function of learned parameters *)
...
let make_variable forward backward arguments =
  ...
  List.iter2_exn initial_partials arguments ~f:(fun p a ->
      if a.volatile then a.descendents <- (s, p) :: a.descendents
  ) ;
  s
...
let random_variable ?(mean = 0.) ?(standard_deviation = 1.) () =
  let rec s =
    {
      gradient = None;
      arguments = [];
...
let ( +& ) = make_binary_variable ( +. ) (fun _ _ -> [ 1.; 1. ])
let ( -& ) = make_binary_variable ( -. ) (fun _ _ -> [ 1.; -1. ])
let ( *& ) = make_binary_variable ( *. ) (fun a b -> [ b; a ])

let ( /& ) =
  make_binary_variable ( /. ) (fun a b -> [ 1. /. b; 0. -. (a /. (b *. b)) ])

let power =
  make_binary_variable ( ** ) (fun a b ->
      [ b *. (a ** (b -. 1.)); (a ** b) *. log a ]
  )

let logarithm = make_unitary_variable log (fun a -> [ 1. /. a ])
let exponential = make_unitary_variable exp (fun a -> [ exp a ])
let square = make_unitary_variable (fun a -> a *. a) (fun a -> [ 2. *. a ])
let square_root = make_unitary_variable sqrt (fun a -> [ 0.5 /. sqrt a ])
...
let rec differentiate z =
  ...
  | None ->
      let g =
        List.fold_right z.descendents ~init:0.0 ~f:(fun (d, partial) a ->
            a +. (differentiate d *. partial)
        )
      in
      z.gradient <- Some g ;
      g
...
let rec run_optimizer opt ?(update = 1000) ?(iterations = 10000) parameters loss
  ...
  if iterations = 0 then
    ...
    run_optimizer opt ~update ~iterations:(iterations - 1) parameters loss
  )

let restarting_optimize
    opt
    ?(update = 1000)
    ?(attempts = 1)
    ?(iterations = 10000)
  ...
  |> List.map ~f:(fun _ ->
         ...
         run_optimizer opt ~update ~iterations parameters loss
     )
  |> fold1 min

let gradient_descent ?(lr = 0.001) = List.map ~f:(fun dx -> ~-.(lr *. dx))

let rprop ?(lr = 0.1) ?(decay = 0.5) ?(grow = 1.2) =
...
let rec polymorphic_loss_pairs : tp -> 'a -> 'b -> (variable * variable) list =
  function
  ...
  | TCon ("list", [ tp ], _) ->
      ...
      magical (fun p y ->
          try List.fold2_exn p y ~init:[] ~f:(fun a _p _y -> a @ e _p _y) with
          | _ -> raise DifferentiableBadShape
      )
  | t -> raise (Failure ("placeholder_data: bad type " ^ string_of_type t))

let rec polymorphic_sse ?(clipOutput = None) ?(clipLoss = None) t =

...
let string_of_vector = function
  | Vector (x, y) -> "(" ^ string_of_int x ^ "," ^ string_of_int y ^ ")"
...
let string_of_command = function
  ...
  | Rectangle (a, b) ->
      "Rectangle(" ^ string_of_vector a ^ ", " ^ string_of_vector b ^ ")"
  | Line (a, b) ->
      "Line(" ^ string_of_vector a ^ ", " ^ string_of_vector b ^ ")"
...
let primitive_circle =
  primitive "circle" (tcoordinate @> ttrace) (fun v -> [ Circle v ])

let primitive_rectangle =
  primitive "rectangle"
...
(* placeholders *)
let primitive_coefficient_placeholder =
  primitive "COEFFICIENT" tcoefficient None
...
let make_empty_guess () =
  ...
  in
  { rectangles = g (); circles = g (); lines = g () }
...
let score_latex output =
  ...
  let pair_guesses s1 s2 =
    match (s1, s2) with
    | Circle (Vector (x1, y1)), Circle (Vector (x2, y2)) ->
    ...
    | ( Rectangle (Vector (x1, y1), Vector (x2, y2)),
        Rectangle (Vector (a1, b1), Vector (a2, b2)) ) ->
  ...
  and single_guesses = function
    ...
    | Line (Vector (x1, y1), Vector (x2, y2)) ->
        g.lines.positions <-
          Vector (x1, y1) :: Vector (x2, y2) :: g.lines.positions ;
        g.lines.x_intercept <- x1 :: x2 :: g.lines.x_intercept ;
        g.lines.y_intercept <- y1 :: y2 :: g.lines.y_intercept
    | Rectangle (Vector (x1, y1), Vector (x2, y2)) ->
        g.rectangles.positions <-
          Vector (x1, y1) :: Vector (x2, y2) :: g.rectangles.positions ;
  ...
  let rec random_instantiation ~x ~i expression =

...
let rec occurs (i : int) (t : tp) : bool =
  ...
  then false
    ...
    match t with
    | TID j -> j = i
    | TCon (_, ts, _) -> List.exists ts (occurs i)
...
let instantiate_type k t =
  ...
  let rec instantiate j =
    ...
    then j
      ...
      match j with
      | TID i -> (
          try List.Assoc.find_exn ~equal:(fun a b -> a = b) !substitution i with
          | Not_found ->
...
let unify' context_reference t1 t2 =
  context_reference := unify !context_reference t1 t2
...
let canonical_type t =
  ...
  let rec canon q =
    match q with
    | TID i -> (
        try TID (List.Assoc.find_exn ~equal:( = ) !substitution i) with
        | Not_found ->
...
let rec next_type_variable t =
  ...
  | TCon (_, [], _) -> 0
  | TCon (_, is, _) ->
      List.fold_left ~f:max ~init:0 (List.map is next_type_variable)
...
let rec deserialize_type j =
  ...
  try
    ...
    let a =
      j |> member "arguments" |> to_list |> List.map ~f:deserialize_type
    in
    kind k a
  with
  | _ ->

...
let rec exploration_compression
  ...
  then g
    ...
    Printf.printf "Hit %d/%d of the tasks with a frontier of size %d.\n"
      (frontiers
      |> List.filter ~f:(fun f -> List.length f.programs > 0)
      |> List.length
      )
      (List.length frontiers) frontier_size ;

    Out_channel.flush stdout ;

    (*     estimate_categorized_fragment_grammar (fragment_grammar_of_grammar g) frontiers; *)
    let fragments =
      time_it "Proposing fragments" @@ fun _ ->
      propose_fragments_from_frontiers arity frontiers
    in
    ...
    let gf =
      time_it "Induced grammar" @@ fun _ ->
      induce_fragment_grammar ~lambda ~alpha ~beta fragments frontiers
        (fragment_grammar_of_grammar g)
    in
    ...
    exploration_compression tasks gp frontier_size ~keepTheBest (iterations - 1)

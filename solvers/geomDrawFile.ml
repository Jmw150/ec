...
let print_pos lexbuf =
  ...
  sprintf "(line %d ; column %d)" pos.pos_lnum (pos.pos_cnum - pos.pos_bol)
...
let parse_with_error lexbuf =
  ...
  | GeomLexer.SyntaxError msg ->
      ...
      raise
        (MalformedProgram (sprintf "Error at position %s, %s" pos_string msg))
  | GeomParser.Error ->
      let pos_string = print_pos lexbuf in
...
let _ =
  ...
  try
    ...
    | None -> ()
  with
  | MalformedProgram error_message -> Printf.printf "%s\n" error_message

...
let remove_bad_dreams behavior_to_programs :
    (PolyList.t * (float * program list)) list =
  ...
  Hashtbl.iteri behavior_to_programs ~f:(fun ~key ~data ->
      ...
      |> List.iteri ~f:(fun output_index this_output ->
             ...
             if this_output = PolyValue.None then
               ...
               | None ->
                   Hashtbl.set containers.(output_index) ~key:this_output
  ...
  let is_bad_index i =
    ...
    |> List.iteri ~f:(fun output_index this_output ->
           if this_output = PolyValue.None then
             ...
             | Some others -> (
                 ...
                 | Some d when Int.Set.length d > Int.Set.length others ->
                     dominating := Some others
                 | _ -> ()
               )
       ) ;
    ...
    if nightmare && false then (
      ...
      |> snd
      |> snd
      |> List.iter ~f:(fun p -> p |> string_of_program |> Printf.eprintf "%s\n")
...
let helmholtz_enumeration
    (behavior_hash : program -> (PolyList.t * float) option)
    ?(nc = 1)
    g
    request
    ~timeout
  ...
  let update ~key ~data =
    ...
    | None -> Hashtbl.set behavior_to_programs ~key ~data
    ...
    | Some (l', extra_cost', ps') when extra_cost = extra_cost' && l < l' -> ()
    ...
    | Some (l', extra_cost', ps') ->
        ...
        if not (l = l') then
          Printf.eprintf "costs:\t%f\t%flikelihood:\t%f\t%f\n" extra_cost
            extra_cost' l l' ;
        assert (l = l') ;
        Hashtbl.set behavior_to_programs ~key
          ~data:
            ( l,
              extra_cost,
              ps @ ps' |> List.dedup_and_sort ~compare:compare_program
            )
  in

  (* unused *)
  let merge other = Hashtbl.iteri other ~f:update in
        ...
        enumerate_programs ~extraQuiet:true ~nc
          ...
          (fun p l ->
            if Hashtbl.length behavior_to_programs > maximumSize then
              ...
              | Some (key, extra_cost) ->
                  update ~key ~data:(l, extra_cost, [ p ])
              | None -> ()
          )
        |> List.concat
      in
...
let rec unpack x =
  ...
  try magical (x |> to_int) with
  | _ -> (
      try magical (x |> to_number) with
      | _ -> (
          try magical (x |> to_bool) with
          | _ -> (
              try
              ...
              | _ -> (
                  ...
                  | _ -> raise (Failure "could not unpack")
                )
            )
        )
    )
...
let register_special_helmholtz name handle =
  Hashtbl.set special_helmholtz name handle

let default_hash ?(timeout = 0.001) request inputs :
             ...
             try
               match
                 run_for_interval ~attempts:2 timeout (fun () ->
                     run_lazy_analyzed_with_arguments p input
                 )
               with
...
let string_hash ?(timeout = 0.001) request inputs :
  ...
  fun program ->
    let constant_results =
      (* results from substituting with each constant *)
      constants
      |> List.concat_map ~f:(fun constant ->
...
let tower_hash ?(timeout = 0.001) request inputs :
    program -> (PolyList.t * float) option =
  let open Yojson.Basic.Util in
...
let logo_hash ?(timeout = 0.001) request inputs :
    program -> (PolyList.t * float) option =
  let open Yojson.Basic.Util in
  assert (request = turtle @> turtle) ;
  (* disgusting hack *)
  let costMatters =
    1 = (inputs |> to_list |> List.hd_exn |> to_list |> List.hd_exn |> to_int)
  in
...
let regex_hash ?(timeout = 0.001) request inputs :
    program -> (PolyList.t * float) option =
  let open Yojson.Basic.Util in
  assert (request = tregex @> tregex) ;

  let rec poly_of_regex = function
    | Constant s -> PolyValue.List [ PolyValue.Integer 0; poly_of_string s ]
    ...
    | Alt (p, q) ->
        PolyValue.List [ PolyValue.Integer 4; poly_of_regex p; poly_of_regex q ]
    | Concat (p, q) ->
        PolyValue.List [ PolyValue.Integer 5; poly_of_regex p; poly_of_regex q ]
  ...
  fun expression ->
    if number_of_free_parameters expression > 1 then
      ...
      run_for_interval ~attempts:2 timeout (fun () ->
          let r =
            expression
            |> substitute_constant_regex default_constant
            |> regex_of_program
            |> canonical_regex
          in
          ([ poly_of_regex r ], 0.)

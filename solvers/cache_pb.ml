...
let default_tower_cash_mutable () : tower_cash_mutable = { entries = [] }
...
let rec decode_tower_cash_block d =
  ...
  while !continue__ do
    ...
    | None ->
        ...
        continue__ := false
    | Some (1, Pbrt.Varint) ->
        v.x10 <- Pbrt.Decoder.int32_as_varint d ;
        x10_is_set := true
    | Some (1, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_block), field(1)" pk
    | Some (2, Pbrt.Varint) ->
        v.w10 <- Pbrt.Decoder.int32_as_varint d ;
        w10_is_set := true
    | Some (2, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_block), field(2)" pk
    | Some (3, Pbrt.Varint) ->
        v.h10 <- Pbrt.Decoder.int32_as_varint d ;
        h10_is_set := true
    | Some (3, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_block), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done ;
  if not !h10_is_set then Pbrt.Decoder.missing_field "h10" ;
  if not !w10_is_set then Pbrt.Decoder.missing_field "w10" ;
  if not !x10_is_set then Pbrt.Decoder.missing_field "x10" ;
  ( { Cache_types.x10 = v.x10; Cache_types.w10 = v.w10; Cache_types.h10 = v.h10 }
...
let rec decode_tower_cash_entry d =
  ...
  while !continue__ do
    ...
    | Some (5, Pbrt.Bits32) ->
        v.height <- Pbrt.Decoder.float_as_bits32 d ;
        height_is_set := true
    | Some (5, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_entry), field(5)" pk
    | Some (6, Pbrt.Bits32) ->
        v.stability <- Pbrt.Decoder.float_as_bits32 d ;
        stability_is_set := true
    | Some (6, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_entry), field(6)" pk
    | Some (7, Pbrt.Bits32) ->
        v.area <- Pbrt.Decoder.float_as_bits32 d ;
        area_is_set := true
    | Some (7, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_entry), field(7)" pk
    | Some (8, Pbrt.Bits32) ->
        v.length <- Pbrt.Decoder.float_as_bits32 d ;
        length_is_set := true
    | Some (8, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_entry), field(8)" pk
    | Some (9, Pbrt.Bits32) ->
        v.overpass <- Pbrt.Decoder.float_as_bits32 d ;
        overpass_is_set := true
    | Some (9, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_entry), field(9)" pk
    | Some (10, Pbrt.Bits32) ->
        v.staircase <- Pbrt.Decoder.float_as_bits32 d ;
        staircase_is_set := true
    | Some (10, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash_entry), field(10)"
          pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done ;
  if not !staircase_is_set then Pbrt.Decoder.missing_field "staircase" ;
  if not !overpass_is_set then Pbrt.Decoder.missing_field "overpass" ;
  if not !length_is_set then Pbrt.Decoder.missing_field "length" ;
  if not !area_is_set then Pbrt.Decoder.missing_field "area" ;
  if not !stability_is_set then Pbrt.Decoder.missing_field "stability" ;
  if not !height_is_set then Pbrt.Decoder.missing_field "height" ;
  ( {
...
let rec decode_tower_cash d =
  ...
  while !continue__ do
    ...
    | Some (1, Pbrt.Bytes) ->
        v.entries <-
          decode_tower_cash_entry (Pbrt.Decoder.nested d) :: v.entries
    | Some (1, pk) ->
        Pbrt.Decoder.unexpected_payload "Message(tower_cash), field(1)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done ;
  ({ Cache_types.entries = v.entries } : Cache_types.tower_cash)

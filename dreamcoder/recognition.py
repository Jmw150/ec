...
class GrammarNetwork(nn.Module):
    ...
    def __init__(self, inputDimensionality, grammar):
        ...
        # length of grammar is length of its productions (a list)
        ## productions are produced by enumeration
...
class RecognitionModel(nn.Module):
    ...
    def __init__(
        # {{{
        self,
        featureExtractor,  # varied neural nets, depends on problem, recurrent or convolutional
        grammar,  # (loss, productions, primitives)
        hidden=[64],  # hidden layers of neural networks
        activation="tanh",  # activations between all layers
        rank=None,  # choosing between types of bigram models
        contextual=False,  # |
        mask=False,  # |
        cuda=False,  # cuda api for parallel compute such as gpu and tpu
        previousRecognitionModel=None,  # loading a previous model, instead
        id=0,  # identification number of recognition model, is ensembleSize of default 1
    ):
        ...
        ## anyway, made to match feature extractor size, but only single activation type
        ##
        self._MLP = nn.Sequential(
            *[
                layer
                for j in range(len(hidden))  # default 1
                for layer in [
                    nn.Linear(
                        ([featureExtractor.outputDimensionality] + hidden)[j],
                        hidden[j],  # default 64, arbitrarily?
                    ),
                    activation(),
                ]
            ]
        ...
        # method to measure log loss of output
        self.entropy = Entropy()

        # set output dimension, check that output layers computed correctly
        if len(hidden) > 0:
        ...
        # boolean sets of Contextual x Mask
